...
//	fv = fv*fv*(3.0-2.0*fv);
//	
//	vec2 uv = (p.xy+vec2(37.0,17.0)*p.z) + fv.xy;
//	vec2 rg = textureLod( iChannel0, (uv+ 0.5)/256.0, 0.0 ).yx;
//    	
//	f  = 0.50000*mix(rg.x, rg.y, f.z); q = q*2.02 - vec3(0.0,1.0,0.0)*iTime;
//
//    	x = q;
//	p = floor(x);
//	fv = fract(x);
//	fv = fv*fv*(3.0-2.0*fv);
//	
//	uv = (p.xy+vec2(37.0,17.0)*p.z) + fv.xy;
//	rg = textureLod( iChannel0, (uv+ 0.5)/256.0, 0.0 ).yx;
//	f += 0.25000*mix(rg.x, rg.y, f.z); q = q*2.03 - vec3(0.0,1.0,0.0)*iTime;
//    	x = q;
//	p = floor(x);
//	fv = fract(x);
//	fv = fv*fv*(3.0-2.0*fv);
...
//		vec3 col = mix( vec3(1.0,0.9,0.8), vec3(0.4,0.15,0.1), den ) + 0.05*sin(p);
//		
//		vec4 colr = vec4(col, den);
//		
//		colr.xyz *= mix( 3.1*vec3(1.0,0.5,0.05), vec3(0.48,0.53,0.5), clamp( (pos.y-0.2)/2.0, 0.0, 1.0 ) );
//		
//		colr.a *= 0.6;
//		colr.rgb *= colr.a;
//
//		sum = sum + colr*(1.0 - sum.a);	
//
//		t += 0.05;
//	}
//
...
void main()
{
    ...
    if( iMouse.y<=0.00001 ) mo=vec2(0.0);
    ...
    // dithering	
	t += 0.05*textureLod( iChannel0, gl_FragCoord.xy/iChannelResolution[0].x, 0.0 ).x;
...
//fract(sin(dot(co.xy ,vec2(12.9898,78.233))) * 43758.5453)
	...
	col = clamp( sum.xyz, 0.0, 1.0 );
	//col = raymarch( ro, rd, fragCoord );
