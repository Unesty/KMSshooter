...
*/
		...
		ms = (now.tv_nsec - last_redraw.tv_nsec) / (1000.f * 1000.f);
		ms += 1000.f * (now.tv_sec - last_redraw.tv_sec);
		printf("mouse variables handling: %f\n", ms);
		timespec_get(&now, TIME_UTC);
		last_redraw = now;
#endif
		/*
		...
		*/
		...
		ms = (now.tv_nsec - last_redraw.tv_nsec) / (1000.f * 1000.f);
		ms += 1000.f * (now.tv_sec - last_redraw.tv_sec);
		printf("glUniform4iv(udeck,52,deck);: %f\n", ms);
		timespec_get(&now, TIME_UTC);
		last_redraw = now;
#endif
		//time+=0.01;
		...
		ms = (now.tv_nsec - last_redraw.tv_nsec) / (1000.f * 1000.f);
		ms += 1000.f * (now.tv_sec - last_redraw.tv_sec);
		printf("glUniform2fv(gl.mouse, 1, cur);: %f\n", ms);
		timespec_get(&now, TIME_UTC);
		last_redraw = now;
#endif
		...
		ms = (now.tv_nsec - last_redraw.tv_nsec) / (1000.f * 1000.f);
		ms += 1000.f * (now.tv_sec - last_redraw.tv_sec);
		printf("glDrawArrays(GL_TRIANGLES, 0, 14);: %f\n", ms);
		timespec_get(&now, TIME_UTC);
		last_redraw = now;
#endif
		...
		ms = (now.tv_nsec - last_redraw.tv_nsec) / (1000.f * 1000.f);
		ms += 1000.f * (now.tv_sec - last_redraw.tv_sec);
		printf("eglSwapBuffers(gl.display, gl.surface);: %f\n", ms);
		timespec_get(&now, TIME_UTC);
		last_redraw = now;
#endif
		//glClear(GL_COLOR_BUFFER_BIT);
...
//				DRM_MODE_PAGE_FLIP_EVENT, &waiting_for_flip);
		//fbstick=1-fbstick;
		//asm("mov $35,%rax; lea tspcl(%rip),%rdi; mov $0, %rsi; syscall");
		/*
