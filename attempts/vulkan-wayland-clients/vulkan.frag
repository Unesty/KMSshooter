...
void main() {
	...
	//fragColor = vec4(ubo.t, 0.3, 0.8, 1);
	fragColor = vec4(gl_PointCoord.x>(ubo.t-0.1)&&gl_PointCoord.x<ubo.t&&gl_PointCoord.y>0.3&&gl_PointCoord.y<0.7,0,0,1);
	//uv.x<200.0*uv.x>0.1*uv.y>0.0*uv.y<0.8
}
