...
#include "main.frag.inc"
	...
	if (!yarivinit) {
		...
		out_spirv_size = yariv_decode_size(in_yariv, in_yariv_size) + 2;
		out_spirv = malloc(out_spirv_size);
		printf("unpack shader...\n");
		yariv_decode(out_spirv, out_spirv_size, in_yariv, in_yariv_size);
		printf("load shader...\n");
	}
	demo->frag_shader_module = demo_prepare_shader_module(demo, out_spirv, out_spirv_size - 2); //-2 to fix uint8 size
	if (!yarivinit)printf("shader loaded...\n");
...
void demo_create_window(struct demo *demo) {
	...
	/*
	char *demoname="vklauncher";
	demo->xdg_wm_base =
		wl_registry_bind(demo->registry, demoname, &xdg_wm_base_interface, 1);
	*/
	if(demo->xdg_wm_base==NULL){
		printf("no xdg_wm_base\n");
	}
	demo->xdg_surface =
		xdg_wm_base_get_xdg_surface(demo->xdg_wm_base, demo->window);
	demo->xdg_toplevel = xdg_surface_get_toplevel(demo->xdg_surface);

	xdg_surface_add_listener(demo->xdg_surface, &xdg_surface_listener, NULL);
	xdg_toplevel_add_listener(demo->xdg_toplevel, &xdg_toplevel_listener, NULL);
...
int main(int argc, char **argv) {
	...
	if((err=volkInitialize())) {
		ERR_EXIT("It's strange, but", "volkInitialize() failed");
	}
	...
	*/
	...
	vkGetPhysicalDeviceMemoryProperties(demo->gpu, &demo->memory_properties);
	demo_prepare(&demo);
	xdg_surface_set_window_geometry(demo.xdg_surface, 0, 0, demo.width, demo.height);
	...
	wl_seat_destroy(demo->seat);
	//wl_shell_surface_destroy(demo->shell_surface);
	xdg_shell_surface_destroy(demo->xdg_surface);
	wl_surface_destroy(demo->window);
	//wl_shell_destroy(demo->shell);
	xdg_shell_destroy(demo->xdg_shell);
	wl_compositor_destroy(demo->compositor);
	wl_registry_destroy(demo->registry);
	wl_display_disconnect(demo->display);
#endif
