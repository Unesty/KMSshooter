...
static int modeset_create_fb(int fd, struct modeset_dev *dev)
{
	...
	//asm("mov $9,%rax;xor %rdi,%rdi;lea $200000,%rsi;mov %0,%rdx;");
...
static void modeset_draw(void)
{
	...
	i=j=k=off=blink=jt=kt=fede=0;
	int mx,my,cx,cy;
	mx=my=cx=cy=0;
	struct modeset_dev *iter;
	char bstart,bsett,bquit;

	int cxaddr;
	char cxfd;
	//mmap(cxaddr,4,PROT_READ,MAP_ANONYMOUS|MAP_SHARED,cxfd,0);
...
*/
	/*
	for (off=0; off< 768*1366-1;off++){
		*(uint32_t*)&iter->map[off]=rand() % 0xff;
	}
	*/
...
/*
	int bpx, bpy;
	for(bpx=1;bpx<63;bpx++){
		for(bpy=1;bpy<63;bpy++){
			//box[bpx][bpy]= (bpx*bpy/(bpx+bpy) << 16) | (bpx*bpy/(bpx+bpy) << 8) | bpx*bpy/(bpx+bpy);
			box[bpx][bpy]=bpx*bpy/(bpx+bpy);
		//	(r << 16) | (g << 8) | b;
		}
	}
*/
	...
	char *mouse;
	//read(fede,&mouse,3);
	mouse = mmap(0,3,PROT_READ|PROT_WRITE,MAP_ANONYMOUS|MAP_SHARED,cxfd,0);
	mouse[0]=0;
	mouse[1]=0;
	mouse[2]=0;
	if((cpid=syscall(56,0,0))==0){
		close(4);
		close(5);
		close(6);
...
loop:
		...
		if(ox==oy){
			if(ox<0){
				for(int dcx=pcx;(dcx+dcy)>(pcx-mouse[1]+pcy+mouse[2]);){
					if(dcx>cx)
						dcx--;
					if(dcy>cy)
						dcy--;
					for(int bpx=0;bpx<63;bpx++){
						for(int bpy=0;bpy<63;bpy++){
							prop[bpx+dcx][bpy+dcy]=box[bpx][bpy];
						}
					}
				}
			}else{	
		...
		//no cursor smothing
/*
		for(int bpx=0;bpx<CURSOR_SIZE;bpx++){
			for(int bpy=0;bpy<CURSOR_SIZE;bpy++){
				//prop[bpx+cx][bpy+cy]=box[bpx][bpy];
			}
		}
*/
		//write(1," slw\n",6);
		//for(int off=768*cx+cy*4;off<CURSOR_SIZE^2;off++){
		//	iter->map[cx+cy+90] = (99 << 16) | (200 << 8) | 250;
		//}
#endif
		...
		if(mouse[0]==9){
			if(cx>90&&cx<500){
				if(cy>90&&cy<200){
					for(int btnx=90;btnx<500;btnx++){
						for(int btny=90;btny<200;btny++){
							bsett=0;
							//prop[btnx][btny]=900;			
							//*(uint32_t*)&iter->map[btnx*iter->width+btny] = (900 << 8);
						}
					}
				}
				if(cy>310&&cy<400){
					for(int btnx=90;btnx<500;btnx++){
						for(int btny=310;btny<400;btny++){
							bsett=100;
							//prop[btnx][btny]=900;
							//*(uint32_t*)&iter->map[btnx*iter->width+btny] = (900 << 16);
						}
					}
				}
				if(cy>510&&cy<600){
					for(int btnx=90;btnx<500;btnx++){
						for(int btny=510;btny<600;btny++){
							//*(uint32_t*)&iter->map[btnx*iter->width+btny] = 900;
							return 0;
							//prop[btnx][btny]=900;
						}
					}
...
*/
					...
					//b=prop[k][j];
					//printf("%d\n",bluu);
					*(uint32_t*)&iter->map[off] = ((k>90)*(k<500)*(j>310)*(j<400)*30+bsett << 16) | (int32_t)(sin((float)i/10)*128+128) << 8 | (k<cx+CURSOR_SIZE/2)*(k>cx-CURSOR_SIZE/2)*(j<cy+CURSOR_SIZE/2)*(j>cy-CURSOR_SIZE/2)*90; //(int)(100*sin(off/900000*(cy/120));
