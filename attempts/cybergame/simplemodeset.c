...
//------------------------------------------------------------------------------
	...
	int cpid=syscall(56,0,0,0);
	char mfd;
	char *mouse;
	mmap(mouse,3,PROT_READ|PROT_WRITE,MAP_SHARED|MAP_ANONYMOUS,0,0);
	if(cpid==-1){
		printf("fork error\n");
	}else if(cpid==0){
		mfd=open("/dev/input/mouse0",O_RDONLY);
		while(1){
			syscall(0,mfd,&mouse,3);
		}
	}
	int x,y;
	for (i=0;i<6000;i++)
		...
		for (j=0;j<res.count_connectors;j++)
		{
			for (y=0;y<fb_h[j];y++)
				for (x=0;x<fb_w[j];x++)
				{
					...
					*(((uint32_t*)fb_base[j])+location)=col*(float)(mouse[0]+1);
				}
		}
		//usleep(100000);
	}
